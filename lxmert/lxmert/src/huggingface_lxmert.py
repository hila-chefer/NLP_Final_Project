...
class LxmertAttention(nn.Module):
    ...
    def forward(self, hidden_states, context, attention_mask=None, output_attentions=False, head_prune=None):
        ...
        ######## prune heads here - attention_probs: 1 x h x (t+i) x (t+i)
        ####### if head_prune != None, each head will be multiplied by head_prune
        if head_prune is not None:
            if head_prune.size() == torch.Size([1]): # This happens when pruning layers
                attention_probs *= head_prune.item()
            else: # heads_prune is not a scalar - this means we are pruning heads
                head_prune = head_prune.reshape(1, -1, 1, 1)
                attention_probs *= head_prune
...
class LxmertCrossAttentionLayer(nn.Module):
    ...
    def forward(self, input_tensor, ctx_tensor, ctx_att_mask=None, output_attentions=False, head_prune=None):
        output = self.att(input_tensor, ctx_tensor, ctx_att_mask, output_attentions=output_attentions, head_prune=head_prune)
...
class LxmertSelfAttentionLayer(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask, output_attentions=False, head_prune=None):
...
class LxmertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, output_attentions=False, head_prune=None):
        outputs = self.attention(hidden_states, attention_mask, output_attentions=output_attentions, head_prune=head_prune)
...
class LxmertXLayer(nn.Module):
    ...
    def cross_att(
        ...
        text_cross_head_prune=None,
        image_cross_head_prune=None,
    ):
        # Cross Attention
        lang_att_output = self.visual_attention(
            ...
            head_prune=text_cross_head_prune,
        )
        visual_att_output = self.visual_attention(
            ...
            head_prune=image_cross_head_prune
        )
        return lang_att_output, visual_att_output

    def self_att(self, lang_input, lang_attention_mask, visual_input, visual_attention_mask, text_self_head_prune=None,
        image_self_head_prune=None):
        # Self Attention
        lang_att_output = self.lang_self_att(lang_input, lang_attention_mask, output_attentions=False, head_prune=text_self_head_prune)
        visual_att_output = self.visn_self_att(visual_input, visual_attention_mask, output_attentions=False, head_prune=image_self_head_prune)
    ...
    def forward(
        ...
        lang_att_output, visual_att_output = self.cross_att(
            ...
            text_cross_head_prune=text_cross_head_prune,
            image_cross_head_prune=image_cross_head_prune,
        )
        ...
        lang_att_output, visual_att_output = self.self_att(
            ...
            visual_att_output[0],
            visual_attention_mask,
            text_self_head_prune=text_self_head_prune,
            image_self_head_prune=image_self_head_prune,
        )
...
class LxmertEncoder(nn.Module):
    ...
    def forward(
        ...
        text_head_prune=None,
        image_head_prune=None,
    ):
        ...
        for  layer_module in self.layer:
            curr_head_prune = text_head_prune[text_prune_idx] if text_head_prune is not None else None
            text_prune_idx += 1
            l_outputs = layer_module(lang_feats, lang_attention_mask, output_attentions=output_attentions, head_prune=curr_head_prune)
        ...
        for layer_module in self.r_layers:
            curr_head_prune = image_head_prune[image_prune_idx] if image_head_prune is not None else None
            image_prune_idx += 1
            v_outputs = layer_module(visual_feats, visual_attention_mask, output_attentions=output_attentions, head_prune=curr_head_prune)
        ...
        for layer_module in self.x_layers:
            text_cross_head_prune = text_head_prune[text_prune_idx] if text_head_prune is not None else None
            text_prune_idx += 1
            text_self_head_prune = text_head_prune[text_prune_idx] if text_head_prune is not None else None
            text_prune_idx += 1

            image_cross_head_prune = None
            image_self_head_prune = None
            if image_head_prune is not None and image_prune_idx < image_head_prune.shape[0]:
                image_cross_head_prune = image_head_prune[image_prune_idx]
                image_prune_idx += 1
                image_self_head_prune = image_head_prune[image_prune_idx]
                image_prune_idx += 1
...
class LxmertModel(LxmertPreTrainedModel):
    ...
    def forward(
        ...
        encoder_outputs = self.encoder(
            ...
            output_attentions=output_attentions,
            text_head_prune=text_head_prune,
            image_head_prune=image_head_prune,
        )
