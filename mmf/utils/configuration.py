...
class Configuration:
    ...
    def _build_other_configs(self):
        ...
        prune_args = self._build_prune_config(opts_config)
        other_configs = OmegaConf.merge(
            model_config, dataset_config, user_config, args_overrides, prune_args
        )
    ...
    def _build_prune_config(self, config):
        prune = config.prune
        num_of_examples = prune.num_of_examples
        if num_of_examples is None:
            raise KeyError("Required arguments 'prune.num_of_examples' not passed")
        positive_prune = prune.positive_prune
        if positive_prune is None:
            raise KeyError("Required arguments 'prune.positive_prune' not passed")
        is_grad = prune.is_grad
        if is_grad is None:
            raise KeyError("Required arguments 'prune.is_grad' not passed")
        prune_config = OmegaConf.create({'prune.num_of_examples':num_of_examples,'prune.positive_prune': positive_prune, 'prune.is_grad':is_grad })
        prune_config = OmegaConf.create({'prune':prune})
        return prune_config
