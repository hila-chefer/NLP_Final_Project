            ...
            if getattr(self.config, "gradient_checkpointing", False):
                ...
                def create_custom_forward(module):
                    def custom_forward(*inputs):
                        return module(*inputs, output_attentions, heads_prune=(None if heads_prune is None else heads_prune[i])) #TODO should I add heads_prune here too?
                ...
                layer_outputs = layer_module(
                    ...
                    layer_head_mask,
                    output_attentions,
                    heads_prune=(None if heads_prune is None else heads_prune[i]),
                )
...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if heads_prune != None:
            if heads_prune.size() == torch.Size([]): # heads_prune is a scalar - this happens when we prune layers
                attention_probs[0] = heads_prune * attention_probs[0]
            else: # heads_prune is not a scalar - this means we are pruning heads
                attention_probs[0] = heads_prune.unsqueeze(1).unsqueeze(1).to(device="cuda") * attention_probs[0]
