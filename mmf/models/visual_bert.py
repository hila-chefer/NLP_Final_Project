...
class VisualBERTBase(BertPreTrainedModel):
    ...
    def forward(
        ...
        heads_prune: Optional[Tensor] = None,
        ...
        if (
            ...
            encoded_layers = self.encoder(
                text_embedding_output, text_extended_attention_mask, heads_prune=heads_prune
            )
            ...
            encoded_layers = self.encoder(embedding_output, extended_attention_mask, heads_prune=heads_prune)
            sequence_output = encoded_layers[0]
...
class VisualBERTForClassification(nn.Module):
    ...
    def forward(
        ...
        output_dict["heads_prune"] = heads_prune #todo Do I need this?
        return output_dict
...
class VisualBERT(BaseModel):
    ...
    def forward(self, sample_list: Dict[str, Tensor], heads_prune: Tensor = None) -> Dict[str, Tensor]:
